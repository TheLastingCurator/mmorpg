...
Можно добавить второй вид серверов - прокси серверы для раздачи информации игрокам.

Основной игровой сервер в таком случае сможет раздавать на несколько прокси-серверов полное состояние каждый кадр, а каждый из прокси серверов будет обслуживать часть игроков.

Например, если понадобится обслуживать 130 606 игроков, один основной и два прокси-сервера смогут это сделать.

Один из вариантов реализации - мультикаст состояния из игрового сервера на произвольное количество прокси-серверов.
Проблема может быть в том, что мультикаст не везде работает, а UDP потребует использования помехоустойчивого кодирования.

Другой вариант - передача первой половины состояния одному прокси-серверу и второй половины - второму, а далее прокси-сервера могут обменяться половинками состояния. В таком случае с основного сервера уйдет N/N частей состояния, каждый прокси-сервер получит от игрового сервера 1 часть состояния и передаст ее N-1 раз, а взамен получит N-1 оставшуюся часть состояния и таким образом соберет полное состояние.

Если состояние займет 116,250,000 байт в секунду, то есть 5,812,500 каждые 50 мс, а прокси серверов будет 2, от игрового сервера каждый получит 2,906,250 байт, отправит второму прокси-серверу 2,906,250 байт, и получит от второго прокси-сервера тоже 2,906,250 байт. В итоге у прокси-сервера останется свободно исходящие 2,906,250 байт, то есть с задержкой в 50 мс прокси сервер сможет раздавать мир наружу используя 1/2 полосы. Это совершенно бессмысленно, в итоге то что смогут раздать 2 прокси-сервера суммарно равно тому что на них отправил игровой сервер. Разве что процессор можно разгрузить.

Если состояние займет в 2 раза меньше, то каждый прокси-сервер будет тратить только 25 мс на то чтобы получить состояние и еще 25 мс он сможет это состояние раздавать игрокам, что позволит начиная с 2 прокси серверов получать пользу от их наличия.

Чтобы уместить в 2,906,250 байт состояние 65,303 персонажей состояние должно умещаться в 44 байта. Если уместить состояние в 11 байт, то можно передать 261,212 персонажей. Если не все персонажи меняют состояние каждый кадр физики, то можно дополнительно увеличить количество персонажей в мире. Так, при 4 изменениях состояния в секунду количество персонажей можно увеличить еще в 5 раз, до 1,306,060.

Если необходимо обслуживать больше, то необходимо шардирование.
...
Игровой мир будет бесшовным, размер карты 8 км х 8 км, в мире будет всего 262144 (2^18) персонажей, из которых четверть может управляться игроками, а 3/4 - ИИ.
